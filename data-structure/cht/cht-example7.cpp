...
namespace CHT
{
    ...
    ll cross(const Line &p, const Line &q) { return divfloor(p.b-q.b, q.a-p.a); } // dir + : divfloor, dir - : divceil
    ...
    struct CHT
    {
        ...
        ll query(ll x)
        {
            ...
            {
                int mid=lo+hi>>1;
                if(cross(V[mid-1], V[mid]) < x) lo=mid; // dir + : <, dir - : >
                else hi=mid;
            }
        ...
        {
            assert(!V.empty());
            while(V.size()>1 && cross(V[0], V[1]) < x) V.pop_front(); // dir + : <, dir - : >
...
int main()
{
    ...
    for(int i=1; i<=Q; i++)
    {
        ...
        else
        {
            ...
            for(auto &it : cht) ans=max(ans, it.query(x));
            for(auto [a, b] : V) ans=max(ans, a*x+b);
            cout << -ans << "\n";
        }
    }
}